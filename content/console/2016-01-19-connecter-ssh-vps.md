...
{{% tw_alert "success" %}}<i class="fa fa-check-circle"></i> **ASTUCE** : Comme vous pouvez le voir, j’ai une adresse personnalisée pour mon VPS alors que je travail en local (càd sur mon réseau privé derrière ma box Internet). Pour en savoir plus, je vous invite à lire [comment renommer votre serveur sur votre réseau local](/console/donner-nom-dns-a-serveur-local/).{{% /tw_alert %}}
...
{{% tw_alert "info" %}}<i class="fa fa-info-circle"></i> **NOTE :** J’ai choisi une taille de clé RSA de 4096 bits, vous pouvez bien évidement choisir DSA et/ou une autre taille de clé.{{% /tw_alert %}}
...
{{% tw_alert "info" %}}<i class="fa fa-info-circle"></i> **NOTE** : Il est possible de désactiver l’authentification par mot de passe sur le VPS afin que seuls les clients avec une clé soient autorisés à se connecter. Pour cela il faut modifier la valeur de `PasswordAuthentication` à `no`.{{% /tw_alert %}}
